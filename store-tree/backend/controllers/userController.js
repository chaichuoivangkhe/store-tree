...
const registerUser = async (req,res) =>{
    ...
    try {
        ...
        // kiểm tra password có đủ mạnh hay và email có đúng định dạng hay không
        if (!validator.isEmail(email)) {
            return res.json({success:false,message:"Hãy nhập đúng định dạng email"})
        }

        if (password.length<8) {
            return res.json({success:false,message:"Hãy nhập mật khẩu mạnh"})
        }

        // bâm mật khẩu người dùng
